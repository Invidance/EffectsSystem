...
{
	m_mask_image = GenImageColor(getWidth(), getHeight(), BLANK);
	
	ImageDrawTextEx(&m_mask_image, *Application::m_instance->getFont(), in_text, { getWidth() / 2.f - MeasureTextEx(*Application::m_instance->getFont(), in_text, in_font_size, 2).x / 2.f, getHeight() / 2 - in_font_size / 2 }, (float)in_font_size, 2, WHITE);
...
{
	in_pos -= getPos();
	in_pos.x = (int)in_pos.x;
	in_pos.y = (int)in_pos.y;
